        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-400 to-teal-600 flex items-center justify-center text-xl">
                🌊
              </div>
              <span
          ...
          <div>
            <h4
              className="text-white font-black text-sm tracking-widest uppercase mb-5"
            ...
            <ul className="space-y-2">
              {footer.quickLinks.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="text-stone-400 hover:text-cyan-400 text-sm transition-colors flex items-center gap-2 group"
          ...
          <div>
            ...
            <div className="flex gap-3 mt-6">
              {["📘","📸","▶️","🐦"].map((icon, i) => (
                <button
                  key={i}
                  className="w-10 h-10 rounded-full bg-stone-800 hover:bg-stone-700 hover:border-cyan-500/30 border border-stone-700 flex items-center justify-center text-sm transition-all duration-200 hover:scale-110"
                >
                  {icon}
                </button>
              ))}
