    ...
    <section id="guidelines" className="bg-stone-50 py-28 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-20">
          ...
          <p className="text-stone-500 text-lg max-w-2xl mx-auto" style={{ fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {guidelines.map((item,i) => (
            <div key={i} className="group bg-white border border-stone-200 hover:border-cyan-300 rounded-2xl p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-cyan-100 relative overflow-hidden">
              ...
              <h3 className="text-stone-900 font-black text-xl mb-3 relative z-10" style={{ fontFamily:"'Syne',sans-serif" }}>{item.title}</h3>
              <p className="text-stone-500 text-sm leading-relaxed relative z-10" style={{ fontFamily:"'DM Sans',sans-serif" }}>{item.text}</p>
            </div>
          ))}
        </div>
      </div>
